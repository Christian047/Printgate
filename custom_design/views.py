...
def design_editor(request):
    ...
    try:
        # Convert to JSON string, handling potential non-serializable cases
        serialized_initial_json = json.dumps(initial_json)
    except TypeError as e:
        print(f"DEBUG: JSON serialization error - {e}")
        # Fallback to default empty canvas
        serialized_initial_json = json.dumps({"version": "5.3.1", "objects": [], "background": "#ffffff"})
